...
'' \
'**Beispiel:** Eine Webanwendung braucht vielleicht:' \
'* einen Webserver' \
...
'' \
'Mit Docker kann diese Umgebung kontrollierter bereitgestellt werden. Dadurch läuft eine Anwendung auf verschiedenen Systemen oft gleich oder zumindest sehr ähnlich.' \
'' \
'---' \
'' \
...
'' \
'Mit Docker beschreibt man die Umgebung genauer. Dadurch wird ein Projekt leichter startbar, testbar und dokumentierbar.' \
'' \
'---' \
'' \
...
'| **Isolation** | prozessbasiert | starker getrennt |' \
...
'## Docker und Linux' \
'' \
'Docker läuft sehr stark im Linux-Umfeld. Viele Docker-Grundlagen hängen mit Linux zusammen:' \
'* Prozesse' \
'* Dateisystem' \
'* Rechte' \
'* Netzwerke' \
...
'* Ressourcen' \
'' \
'Deshalb ist Linux-Wissen für Docker sehr hilfreich. Wer Linux-Grundlagen versteht, versteht Docker deutlich leichter.' \
'' \
'---' \
'' \
'## Docker und Git' \
...
'' \
'Ein einzelner Container kann mit `docker run` gestartet werden. Mehrere zusammengehörende Container verwaltet man oft mit Docker Compose.' \
'' \
'**Beispiel:** Webanwendung + Datenbank + Admin-Tool' \
'' \
...
'' \
'Container selbst sind nicht als dauerhafter Speicher gedacht. Wenn ein Container gelöscht wird, können Daten im Container verloren gehen. Für dauerhafte Daten nutzt man Volumes.' \
'' \
'**Typische Beispiele:**' \
'* Datenbankdaten' \
'* Uploads' \
...
'' \
'Container müssen oft miteinander kommunizieren (z. B. Web-App $\rightarrow$ Datenbank, Adminer $\rightarrow$ PostgreSQL, Frontend $\rightarrow$ Backend).' \
'' \
...
'```' \
'' \
'In Docker Compose bekommen Services oft automatisch ein gemeinsames Netzwerk. Dann können sie sich über Servicenamen erreichen.' \
'' \
'---' \
'' \
